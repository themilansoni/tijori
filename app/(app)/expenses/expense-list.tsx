  ...
  if (transactions.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-border p-10 text-center">
        <p className="text-muted">No expenses match these filters yet.</p>
      </div>
    );
  }
          ...
          <tbody className="divide-y divide-border">
            {transactions.map((t) => (
              <tr key={t.id} className="bg-surface">
                ...
                <td className="px-4 py-3 text-right font-semibold">{fmtCurrency(t.amount)}</td>
                <td className="px-4 py-3 text-muted">{t.payment_method || "—"}</td>
                <td className="px-4 py-3">
                  <RowActions transaction={t} categories={categories} />
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {transactions.map((t) => (
          <div key={t.id} className="rounded-xl border border-border bg-surface p-4">
            <div className="flex items-start justify-between">
              ...
              <div className="text-right font-bold">{fmtCurrency(t.amount)}</div>
            </div>
  ...
  return (
    <div className="flex items-center gap-3 text-xs">
      <Modal
        ...
        title="Edit expense"
      >
        <ExpenseForm categories={categories} transaction={transaction} />
      </Modal>
      <ConfirmButton
